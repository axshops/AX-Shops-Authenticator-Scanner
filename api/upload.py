    ...
    def do_POST(self):
        try:
            ...
            for idx in range(1, 8):
                step_key = f'step{idx}'
                if step_key not in form:
                    self._set_headers(400)
                    self.wfile.write(json.dumps({"error": f"Missing {step_key}"}).encode())
                    return

                file_item = form[step_key]
                if not file_item.file:
                    self._set_headers(400)
                    self.wfile.write(json.dumps({"error": f"{step_key} is not a file"}).encode())
                    return
                ...
                if not content_type.startswith('image/'):
                    self._set_headers(400)
                    self.wfile.write(json.dumps({"error": f"Step {idx} is not an image"}).encode())
                    return

                filename = file_item.filename or f'step{idx}.jpg'
                ext = os.path.splitext(filename)[1].lower()
                if ext not in ['.jpg', '.jpeg', '.png', '.webp']:
                    self._set_headers(400)
                    self.wfile.write(json.dumps({"error": f"Step {idx} invalid extension"}).encode())
                    return
